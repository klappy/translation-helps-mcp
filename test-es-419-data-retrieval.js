  ...
  try {
    ...
    tests.push({
      ...
      test: async () => {
        ...
        return {
          ...
          hasData: !result?.includes('Tool endpoint failed') && !result?.includes('400'),
        };
      },
    });
    ...
    tests.push({
      ...
      test: async () => {
        ...
        return {
          ...
          hasNotes: result?.notes && Array.isArray(result.notes) && result.notes.length > 0,
    ...
    tests.push({
      ...
      test: async () => {
        ...
        return {
          success: result && typeof result === 'object',
          hasQuestions: result?.questions && Array.isArray(result.questions) && result.questions.length > 0,
    ...
    tests.push({
      ...
      test: async () => {
        ...
        return {
          success: result && typeof result === 'object',
          hasLinks: result?.links && Array.isArray(result.links) && result.links.length > 0,
    ...
    tests.push({
      ...
      test: async () => {
        ...
        return {
          success: result && typeof result === 'object',
          hasArticles: result?.articles && Array.isArray(result.articles) && result.articles.length > 0,
    ...
    for (const test of tests) {
      try {
        ...
        if (result.success && result.hasData) {
          ...
          if (result.dataLength) console.log(`     Response length: ${result.dataLength} chars`);
          if (result.notesCount) console.log(`     Notes found: ${result.notesCount}`);
          if (result.questionsCount) console.log(`     Questions found: ${result.questionsCount}`);
          if (result.linksCount) console.log(`     Links found: ${result.linksCount}`);
          if (result.articlesCount) console.log(`     Articles found: ${result.articlesCount}`);
          if (result.preview) console.log(`     Preview: ${result.preview.substring(0, 100)}...`);
    ...
    results.forEach(r => {
      ...
      if (r.questionsCount !== undefined) console.log(`   Questions: ${r.questionsCount}`);
      if (r.linksCount !== undefined) console.log(`   Links: ${r.linksCount}`);
      if (r.articlesCount !== undefined) console.log(`   Articles: ${r.articlesCount}`);
    });
