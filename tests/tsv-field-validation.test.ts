...
describe('TSV Field Validation - Exact Field Mapping', () => {
  ...
  describe('Translation Notes Field Validation', () => {
    test('TN TSV has exactly 7 columns that must ALL be accessible', () => {
      ...
      expect(expectedMapping.Tags).toBe('rc://*/ta/man/translate/figs-abstractnouns');
  ...
  describe('Field Preservation Requirements', () => {
    test('MUST preserve all TSV columns in JSON response', () => {
      const requirements = {
        translationNotes: {
          tsvColumns: 7,
          requiredFields: ['Reference', 'ID', 'Tags', 'SupportReference', 'Quote', 'Occurrence', 'Note'],
          jsonFields: ['reference', 'id', 'tags', 'supportReference', 'quote', 'occurrence', 'note']
        },
        translationQuestions: {
          tsvColumns: 7,
          requiredFields: ['Reference', 'ID', 'Tags', 'Quote', 'Occurrence', 'Question', 'Response'],
          jsonFields: ['reference', 'id', 'tags', 'quote', 'occurrence', 'question', 'response']
        },
        translationWordLinks: {
          tsvColumns: 6,
          requiredFields: ['Reference', 'ID', 'Tags', 'OrigWords', 'Occurrence', 'TWLink'],
          jsonFields: ['reference', 'id', 'tags', 'origWords', 'occurrence', 'twLink']
        }
      };
