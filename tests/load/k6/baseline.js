...
export default function() {
  ...
  if (Math.random() < 0.01) { // 1% sampling
    console.log(`[${endpoint.name}] ${response.status} ${response.timings.duration}ms ${response.body.length}B`);
  }
...
export function handleSummary(data) {
  return {
    ...
    'baseline-load-test-summary.txt': textSummary(data, { indent: ' ', enableColors: true })
  };
}
