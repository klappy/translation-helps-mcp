...
async function fetchDCSScripture(reference: string) {
  const url = 'https://git.door43.org/unfoldingWord/en_ult/raw/branch/master/56-TIT.usfm';
...
async function fetchDCSNotes(reference: string) {
  const url = 'https://git.door43.org/unfoldingWord/en_tn/raw/branch/master/tn_TIT.tsv';
  const response = await fetch(url);
...
async function fetchDCSQuestions(reference: string) {
  const url = 'https://git.door43.org/unfoldingWord/en_tq/raw/branch/master/tq_TIT.tsv';
  const response = await fetch(url);
  const tsv = await response.text();
...
describe('DCS Data Validation', () => {
  ...
  it('should return the exact scripture text from DCS', async () => {
    // Fetch from our API
    const ourResponse = await fetch(`${BASE_URL}/api/fetch-scripture?reference=Titus%201:1`);
    ...
    if (ourData.scriptures) {
      const ult = ourData.scriptures.find((s: any) => s.translation?.includes('Literal Text'));
      ourText = ult?.text || '';
    }
    
    // Validate
    const matches = ourText.includes('Paul, a servant of God') && dcsText?.includes('Paul, a slave of God');
  ...
  it('should return the exact translation notes from DCS', async () => {
    // Fetch from our API
    const ourResponse = await fetch(`${BASE_URL}/api/fetch-translation-notes?reference=Titus%201:1`);
    const ourData = await ourResponse.json();
    ...
    // Validate count
    const ourNotesCount = ourData.verseNotes?.length || ourData.notes?.length || 0;
    ...
    validationResults.push({
      ...
      differences: ourNotesCount !== dcsNotesCount ? 
  ...
  it('should return the exact translation questions from DCS', async () => {
    // Fetch from our API
    const ourResponse = await fetch(`${BASE_URL}/api/fetch-translation-questions?reference=Titus%201:1`);
    const ourData = await ourResponse.json();
    ...
    expect(ourFirstQ?.question).toBe("What was Paul's purpose in his service to God?");
  });
