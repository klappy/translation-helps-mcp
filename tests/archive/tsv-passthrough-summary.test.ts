...
1:1\trtc9\trc://*/ta/man/translate/figs-abstractnouns\tκατὰ πίστιν\tThe words\t1\tare abstract nouns.`;
      ...
      console.log(
        "JSON has tags?",
        "tags" in actualJsonResponse ? "YES" : "NO ❌",
      );
      console.log("TSV Note field:", tsvRow.Note);
      console.log(
        "JSON note field:",
        actualJsonResponse.note,
        "❌ WRONG VALUE",
      );
    });
...
1:1\ttrr8\tname\tΠαῦλος\t1\trc://*/tw/dict/bible/names/paul`;
      ...
      console.log(
        "Must preserve Unicode exactly:",
        tsvRow.OrigWords === "Παῦλος" ? "YES ✅" : "NO ❌",
      );
    });
  });
  ...
  describe("Summary of Issues", () => {
    ...
    test("ISSUE 2: Fields are lost in JSON response", () => {
      ...
      const lostFields = tsvFields.filter(
        (f) => !jsonFields.some((j) => j.toLowerCase() === f.toLowerCase()),
      );

      expect(lostFields).toEqual([
        "ID",
        "Tags",
        "SupportReference",
        "Occurrence",
