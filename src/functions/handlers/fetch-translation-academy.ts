 ...
 */
export const fetchTranslationAcademyHandler: PlatformHandler = async (request) => {
...
async function fetchTAData(
  ...
  try {
    ...
    if (modules.length === 0) {
      console.log(`No TA content found for ${language}/${organization}, using sample content`);
