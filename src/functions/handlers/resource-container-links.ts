...
export const resourceContainerLinksHandler: PlatformHandler = async (request) => {
  ...
  try {
    ...
    if (!bypassCache) {
      ...
      if (cachedResult) {
        console.log(`🚀 RC Links cache HIT for: ${cacheKey}`);
        return {
          statusCode: 200,
          headers: {
            "Content-Type": "application/json",
    ...
    if (!bypassCache) {
      ...
      console.log(`💾 Cached RC Links response: ${cacheKey}`);
    }
    ...
    console.error("RC Links error:", error);
...
async function fetchRCLinks(
  ...
  try {
    ...
    console.error("Error fetching RC links:", error);
    return null;
  }
}
...
function parseRCManifest(
  ...
  try {
    ...
    console.error("Error parsing RC manifest:", error);
    return null;
  }
}
