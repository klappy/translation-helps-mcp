...
export interface ExportOptions {
  outputPath?: string;
  compress?: number; // 1-9, default 6
  split?: number; // Size in bytes to split files
}
...
export class ResourceTransfer {
  ...
  async exportLanguage(
    ...
    try {
      ...
      const tempDir =
        options.outputPath || path.join(this.exportsDir, `temp-${Date.now()}`);
      ...
      logger.info(
        `📦 Package contains ${files.length} resources (${this.formatBytes(manifest.resources.reduce((sum, r) => sum + r.size, 0))})`,
      );
   ...
   * Export entire cache
   */
  async exportAll(_options: ExportOptions = {}): Promise<string | null> {
    try {
      const languages = fs
   ...
   */
  async exportBundle(
    resources: string[], // Format: "lang/type" e.g., ["en/ult", "en/tn"]
    ...
    try {
      ...
      logger.info(
        `✅ Created bundle with ${exported.length} resources at ${tempDir}`,
      );
      return tempDir;
    } catch (error) {
   ...
   */
  async validateZip(filePath: string): Promise<boolean> {
    try {
   ...
   */
  async importZip(filePath: string): Promise<ImportResult> {
   ...
   */
  async importBulk(directory: string): Promise<ImportResult> {
    const result: ImportResult = {
      success: false,
      imported: [],
      failed: [],
    ...
    try {
      ...
      logger.info(
        `✅ Bulk import: ${result.imported.length} succeeded, ${result.failed.length} failed`,
      );
    } catch (error) {
      result.errors.push({
        file: directory,
