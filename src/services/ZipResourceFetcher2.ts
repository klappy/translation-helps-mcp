...
export class ZipResourceFetcher2 {
    ...
    try {
      ...
      for (let i = 0; i < targets.length; i++) {
        ...
        try {
          ...
          // ZIP sync for indexing is now handled at the ZIP download layer (getOrDownloadZip)
          // Don't add blocking checks here - it adds 400ms+ latency to every cached request
        } catch {
        ...
        // Try ZIP key
        {
          ...
          if (data) {
            ...
            if (!isValidZip) {
              ...
              // R2 sync now handled in r2-storage.ts getZipWithInfo()
              return data;
            }
          }
        }
        ...
        // Try TAR key
        {
          ...
          if (data) {
            ...
            if (!isValidGzip) {
              ...
              // Cache API keys are now versioned - new deploy = cache miss = R2 lookup
              // No sync needed - versioned cache handles this automatically
              return data;
            }
          }
        }
  ...
  async getCleanContent(
    ...
    if (!zipData) {
      logger.warn(
        `[CleanContent] ZIP provider returned null for: ${cleanPath}`,
      );
      return null;
    }

    const zipCacheKey = r2KeyFromUrl(zipUrl).key;
    const rawContent = await this.extractFileFromZip(
      zipData,
      filePath,
      repository,
      zipCacheKey,
    ...
    if (!rawContent) {
      logger.warn(`[CleanContent] Failed to extract: ${cleanPath}`);
      return null;
    }
    ...
    if (!cleanResult.text || cleanResult.text.length === 0) {
      logger.warn(
        `[CleanContent] Cleaning produced empty content: ${cleanPath}`,
      );
      return null;
    }
