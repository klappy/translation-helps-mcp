...
export class ResourceSync {
     ...
     */
    async downloadResource(org, lang, resourceType, onProgress) {
     ...
     */
    async performDownload(org, lang, resourceType) {
        ...
        try {
            ...
            const zipData = await zipFetcher.getOrDownloadZip(org, repoName, "master", // Default ref, LocalZipFetcher will resolve actual ref from catalog if available
            null);
            if (!zipData) {
            ...
            const cachedZipPath = path.join(cachedPath, "zips", org, repoName, `${org}_${repoName}_master.zip`);
            ...
            logger.info(`✅ Downloaded ${resourceType} for ${lang} (${(zipData.length / 1024 / 1024).toFixed(2)} MB)`);
        }
        catch (error) {
     ...
     */
    async downloadAll(lang, org = "unfoldingWord", onProgress) {
        const status = {
            language: lang,
            resources: [],
            totalSize: 0,
        ...
        for (let i = 0; i < this.RESOURCE_TYPES.length; i += concurrency) {
            ...
            await Promise.all(batch.map(async (resourceType, idx) => {
                const progressIdx = i + idx;
                const success = await this.downloadResource(org, lang, resourceType, (progress) => {
        ...
        logger.info(`✅ Sync completed for ${lang} in ${((status.endTime - status.startTime) / 1000).toFixed(1)}s`);
        return status;
    }
    /**
     * Sync resources (update if outdated)
     */
    async syncResources(lang, org = "unfoldingWord") {
     ...
     */
    updateMetadata(resource, status) {
        try {
            let metadata = {};
