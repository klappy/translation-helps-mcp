...
export class LocalZipFetcher {
    ...
    constructor(cacheDir, tracer) {
        ...
        this.tracer =
            tracer || new EdgeXRayTracer(`local-zip-${Date.now()}`, "LocalZipFetcher");
    ...
    async getOrDownloadZip(organization, repository, ref, zipballUrl) {
        try {
            ...
            if (!zipballUrl && !ref) {
                logger.error(`[LocalZipFetcher] No zipballUrl or ref provided for ${organization}/${repository}`);
                return null;
            }
            const zipUrl = zipballUrl ||
            ...
            if (fs.existsSync(localPath)) {
                try {
                    ...
                    const isValidZip = cachedData.length >= 1024 &&
                        cachedData[0] === 0x50 &&
                        cachedData[1] === 0x4b; // PK header
                    const isValidGzip = cachedData.length >= 1024 &&
                        cachedData[0] === 0x1f &&
                        cachedData[1] === 0x8b; // GZIP header
                    if (isValidZip || isValidGzip) {
                        logger.info(`✨ Local cache HIT for ${organization}/${repository} (${(cachedData.length / 1024 / 1024).toFixed(2)} MB)`);
                        ...
                        logger.warn(`⚠️ Cached ZIP is corrupted, deleting and re-downloading`);
            ...
            if (!response.ok) {
                logger.info(`[LocalZipFetcher] Initial ZIP fetch failed: ${response.status} ${response.statusText}`);
                // Prefer plain tag tar.gz first
                let tarResp = await trackedFetch(this.tracer, tarUrl, {
                    headers: this.getClientHeaders(),
                });
                if (!tarResp.ok) {
                    logger.info(`[LocalZipFetcher] TAR.GZ fetch also failed: ${tarResp.status} ${tarResp.statusText}`);
                    ...
                    if (match?.[1]) {
                        ...
                        if (!tarResp.ok) {
                            logger.error(`[LocalZipFetcher] Immutable link also failed: ${tarResp.status}`);
                            return null;
                        }
                    }
                    else {
                        logger.error(`[LocalZipFetcher] No immutable link found, giving up`);
                        return null;
                    }
                }
                else {
                    logger.info(`[LocalZipFetcher] Successfully fetched TAR.GZ from: ${tarUrl}`);
                }
                response = tarResp;
            }
            const buffer = await response.arrayBuffer();
            if (buffer.byteLength < 1024) {
                logger.error(`[LocalZipFetcher] Downloaded file too small (${buffer.byteLength} bytes), not caching`);
                return null;
            }
            const zipData = new Uint8Array(buffer);
            // Validate ZIP/TAR.GZ
            const isValidZip = zipData.length >= 1024 && zipData[0] === 0x50 && zipData[1] === 0x4b;
            const isValidGzip = zipData.length >= 1024 && zipData[0] === 0x1f && zipData[1] === 0x8b;
            if (!isValidZip && !isValidGzip) {
                logger.error(`[LocalZipFetcher] Downloaded file is not a valid ZIP or TAR.GZ`);
                return null;
            }
            ...
            logger.info(`✅ Downloaded and cached ZIP: ${organization}/${repository} (${(zipData.length / 1024 / 1024).toFixed(2)} MB)`);
            this.tracer.addApiCall({
                url: zipUrl,
    ...
    async extractFileFromZip(zipData, filePath, repository) {
        try {
            ...
            const isZip = zipData.length >= 2 && zipData[0] === 0x50 && zipData[1] === 0x4b; // PK
            const isGzip = zipData.length >= 2 && zipData[0] === 0x1f && zipData[1] === 0x8b; // GZ
            if (isZip) {
            ...
            if (isGzip) {
                ...
                logger.warn(`[LocalZipFetcher] TAR.GZ extraction not yet implemented, falling back`);
                return null;
            }
            logger.warn(`[LocalZipFetcher] File not found in ZIP: ${filePath} (tried: ${possiblePaths.join(", ")})`);
            return null;
        }
        catch (error) {
    ...
    async getRawUSFMContent(organization, repository, filePath, ref, zipballUrl) {
        try {
            ...
            const zipData = await this.getOrDownloadZip(organization, repository, ref, zipballUrl);
            if (!zipData) {
                return null;
            }
            // Extract USFM file from ZIP
            const usfmContent = await this.extractFileFromZip(zipData, filePath, repository);
            return usfmContent;
        }
        catch (error) {
