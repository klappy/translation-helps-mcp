  ...
  generateHandler(config: EndpointConfig): GeneratedRouteHandler {
    ...
    const handler: PlatformHandler = async (
      ...
      try {
        ...
        const { data: responseData, cacheStatus: computedCacheStatus } = await this.fetchData(
          config,
          params,
          request
        );
        ...
        const { body, headers } = this.formatResponse(
          ...
          format,
          {
            ...
            dataCacheStatus: computedCacheStatus || this.lastComputedCacheStatus || dataCacheStatus,
            success: true,
            status: 200,
            traceId,
