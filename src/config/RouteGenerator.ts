...
import { extractChapterText, extractVerseText } from "../functions/usfm-extractor";
  ...
  for (const [name, config] of Object.entries(paramConfigs)) {
    ...
    if (name in params && config.validation) {
      ...
      if (config.validation.enum && !config.validation.enum.includes(value)) {
        return {
          valid: false,
          error: `Invalid value for parameter: ${name}. Must be one of: ${config.validation.enum.join(", ")}`,
        };
      }
    }
  }
