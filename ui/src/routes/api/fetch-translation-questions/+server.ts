 ...
 */
function parseTSV(content: string): Array<{ Reference: string; ID: string; Question: string; Response: string }> {
 ...
 */
async function fetchTranslationQuestions(params: Record<string, any>, request: Request): Promise<any> {
	...
	if (search && search.trim().length > 0) {
		...
		console.log(`[fetch-translation-questions] Search filtered ${totalBeforeSearch} to ${results.length}`);
	}

	const baseResponse = createTranslationHelpsResponse(results, reference, language, organization, 'tq', {
