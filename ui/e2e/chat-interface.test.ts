...
test.describe('Chat Interface - AI Bible Assistant', () => {
	...
	test('should handle scripture request with verbatim response', async ({ page }) => {
		...
		const response = page.locator('text=For God so loved the world that he gave his only begotten Son');
	...
	test('should show X-ray panel with tool details', async ({ page }) => {
		...
		// Close X-ray panel
		const closeButton = page.locator('.fixed >> button:has([aria-label*="close"], [aria-label*="Close"], svg)').first();
