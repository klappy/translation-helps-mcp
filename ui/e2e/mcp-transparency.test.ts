...
test.describe('MCP Transparency Features', () => {
	...
	test('should display get_system_prompt tool in MCP Tools', async ({ page }) => {
		...
		const toolCard = page
			...
			.filter({ hasText: 'Get System Prompt' });
		...
		await expect(
			page.locator('text=Include implementation details and validation functions')
		).toBeVisible();
	});
	...
	test('should execute get_system_prompt tool successfully', async ({ page }) => {
		...
		// Wait for response
		await page.waitForSelector('[data-testid="api-response"], pre:has-text("systemPrompt")', {
			timeout: 10000
		});
